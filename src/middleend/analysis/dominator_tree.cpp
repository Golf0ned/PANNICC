...
void DominatorTree::run(mir::Function *f) {
    auto *definition = dynamic_cast<mir::FunctionDefinition *>(f);
    if (!definition)
        return;

    TraversalOrderMap traversal_numbers;
    auto traversal_order = numberPostorder(definition, traversal_numbers);
    ...
    for (auto &[dominator, dominated] : immediate_dominators) {
        immediate_dominated[dominated].push_back(dominator);
    }
}
