...
FunctionDefinition::FunctionDefinition(
    Type type, std::string name, std::vector<std::unique_ptr<Value>> parameters,
...
std::list<std::unique_ptr<BasicBlock>> &FunctionDefinition::getBasicBlocks() {
    return basic_blocks;
}
...
std::string FunctionDefinition::toString() {
    ...
    for (auto iter = basic_blocks.begin(); iter != basic_blocks.end(); iter++) {
...
void ToStringVisitor::visit(InstructionPhi *i) {
    ...
    std::map<BasicBlock *, Value *, decltype(comp)> ordered(pairs.begin(),
                                                            pairs.end(), comp);
    ...
    for (auto iter = ordered.begin(); iter != ordered.end(); iter++) {
        ...
        std::string bb_name = bb_num == -1 ? "entry" : std::to_string(bb_num);
...
void ToStringVisitor::visit(TerminatorCondBranch *t) {
    ...
    std::string bb_true = std::to_string(nir->getNumber(t->getTSuccessor()));
