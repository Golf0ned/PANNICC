...
void SimplifyCFG::run(mir::Function *f) {
    ...
    while (changed) {
        ...
        while (iter != bbs.end()) {
            ...
            auto tryRemoveOrphaned = [&]() {
                if (preds.getSize() || bb == definition->getEntryBlock())
                    return false;
            ...
            auto tryMergeStraightLine = [&]() {
                ...
                for (auto &i : bb->getInstructions()) {
                    assert(!dynamic_cast<mir::InstructionPhi *>(i.get()));
                ...
                pred->setTerminator(bb->getTerminator()->clone());
            ...
            auto tryMergeUncondBr = [&]() {
                auto terminator =
                    dynamic_cast<mir::TerminatorBranch *>(bb->getTerminator());
                ...
                bool succ_has_phi =
                    !succ_insts.empty() && dynamic_cast<mir::InstructionPhi *>(
                ...
                for (auto pred : preds.getEdges()) {
                    ...
                    auto pred_terminator = pred->getTerminator();
