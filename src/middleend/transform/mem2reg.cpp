...
computeDominanceFrontiers(DominatorTree *dt, mir::FunctionDefinition *f) {
    std::unordered_map<mir::BasicBlock *, std::vector<mir::BasicBlock *>> res;
...
void Mem2Reg::run(mir::Function *f) {
    ...
    for (auto &bb : definition->getBasicBlocks()) {
        for (auto &i : bb->getInstructions()) {
            ...
            auto *alloca = dynamic_cast<mir::InstructionAlloca *>(i.get());
            if (alloca) {
                for (auto &[use, _] : alloca->getUses()) {
                    ...
                    if (use_store && dynamic_cast<mir::InstructionAlloca *>(
            ...
            // Mark defs
            auto *store = dynamic_cast<mir::InstructionStore *>(i.get());
            if (store) {
                auto ptr_alloca =
                    dynamic_cast<mir::InstructionAlloca *>(store->getPtr());
                if (!ptr_alloca)
                    continue;
    ...
    for (auto const &[alloca, def_blocks] : defs) {
        ...
        while (!worklist.empty()) {
            ...
            for (auto &bb : dominance_frontiers[block]) {
                ...
                phis[bb][alloca] = std::make_unique<mir::InstructionPhi>(type);
    ...
    // Variable renaming
    //
    std::vector<std::pair<mir::BasicBlock *,
    ...
    while (!worklist.empty()) {
        ...
        while (iter != instructions.end()) {
            ...
            if (load) {
                auto alloca =
                    dynamic_cast<mir::InstructionAlloca *>(load->getPtr());
                if (!alloca || unpromotable.contains(alloca)) {
                    iter++;
                    continue;
                }
    ...
    auto cmp_phi = [&](std::unique_ptr<mir::InstructionPhi> &first,
        ...
        auto first_size = first_preds.size(), second_size = second_preds.size();
        ...
        auto first_iter = first_map.begin(), second_iter = second_map.begin();
    ...
    for (auto &[bb, map] : phis) {
        ...
        for (auto &[_, phi] : map) {
            ...
            case 1: // single branch phi: replace use with value
                ReplaceUsesVisitor ruv(phi.get(),
                                       phi->getPredecessors().begin()->second);
