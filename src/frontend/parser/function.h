...
struct function_header
    : pegtl::seq<type, ignorable, identifier, ignorable, left_paren, ignorable,
                 pegtl::opt<pegtl_comma_list<function_param>>, right_paren> {};
...
template <> struct action<function_param> {
    ...
    static void apply(const Input &in,
        ...
        active_params.push_back({std::move(param_type), std::move(param_name)});
    }
};
