...
struct directive
    : pegtl::sor<pegtl_try<directive_define>, pegtl_try<directive_include>> {};

//
// Rest of TU
//
struct line : pegtl::sor<pegtl::seq<pegtl::opt<whitespace>, directive>, body> {
};
...
template <> struct action<body_identifier> {
    ...
    static void apply(const Input &in, std::string &res) {
        ...
        res += replacement != define_map.end() ? replacement->second : token;
    }
};
