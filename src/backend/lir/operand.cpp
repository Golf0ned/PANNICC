...
std::string Register::toString() { return "%" + ::backend::lir::toString(reg); }
...
ConstrainedRegister::ConstrainedRegister(VirtualRegister *reg,
    ...
    : VirtualRegister(reg->getName(), reg->getSize()), constraint(constraint) {}
...
VirtualRegister *OperandManager::getRegister(std::string name, DataSize size) {
...
Address *OperandManager::getAddress(Register *base, Register *index,
                                    Immediate *scale, Immediate *displacement) {
    addresses.push_back(
