...
std::vector<std::unique_ptr<FunctionInfo>> TreeGenVisitor::getFunctionInfo() {
    ...
    for (size_t param_num = 0; param_num < params.size(); param_num++) {
        ...
        auto src =
            param_num < 6
                ...
                : static_cast<lir::Operand *>(om->getStackArg(param_num - 6));
...
std::unique_ptr<Node> TreeGenVisitor::resolveLeaf(middleend::mir::Value *v) {
...
lir::Operand *TreeGenVisitor::resolveOperand(middleend::mir::Value *v) {
    ...
    if (alloca) {
        ...
        std::list<std::unique_ptr<lir::Instruction>> instructions;
        auto lea = std::make_unique<lir::InstructionLea>(size, address, dst);
...
void TreeGenVisitor::visit(middleend::mir::InstructionBinaryOp *i) {
    ...
    auto op =
        std::make_unique<OpNode>(i->getOp(), std::move(left), std::move(right));
...
void TreeGenVisitor::visit(middleend::mir::InstructionStore *i) {
    ...
    auto store =
        std::make_unique<StoreNode>(std::move(source), std::move(ptr), size);
...
void TreeGenVisitor::visit(middleend::mir::InstructionParallelCopy *i) {
    ...
    while (!worklist.empty()) {
        ...
        while (worklist_iter != worklist.end()) {
            ...
            auto copy = std::make_unique<lir::InstructionMov>(size, src, dst);
...
void TreeGenVisitor::visit(middleend::mir::TerminatorBranch *t) {
    ...
    if (succ != next_block) {
        auto jmp = std::make_unique<lir::InstructionJmp>(resolveLabel(succ));
...
void TreeGenVisitor::visit(middleend::mir::TerminatorCondBranch *t) {
    ...
    auto cmp =
        std::make_unique<lir::InstructionCmp>(size, om->getImmediate(0), cond);
