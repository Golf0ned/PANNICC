...
bool matchIS(RegisterNode *node, RegisterNode **index_res,
    ...
    auto scale = dynamic_cast<ImmediateNode *>(left_reg ? op->getRight().get()
                                                        : op->getLeft().get());
    if (!scale)
        return false;
...
lir::Operand *Tile::resolveOperand(Node *node, std::vector<Node *> &worklist) {
...
StoreTile::apply(std::vector<Node *> &worklist) {
    ...
    auto store_asm = std::make_unique<lir::InstructionMov>(tile_size, src, dst);
...
BinOpTile::apply(std::vector<Node *> &worklist) {
    ...
    if (op == middleend::mir::BinaryOp::SDIV) {
        ...
        auto in_asm = std::make_unique<lir::InstructionMov>(size, left, eax);
        assembly.push_back(std::move(in_asm));

        auto extend_asm = std::make_unique<lir::InstructionConvert>(size, size);
        assembly.push_back(std::move(extend_asm));

        auto div_asm =
            std::make_unique<lir::InstructionSpecialOp>(bin_op, size, right);
        assembly.push_back(std::move(div_asm));

        auto out_asm = std::make_unique<lir::InstructionMov>(size, eax, dst);
        assembly.push_back(std::move(out_asm));
    } else {
        auto mov_asm = std::make_unique<lir::InstructionMov>(size, left, dst);
        ...
        auto op_asm = std::make_unique<lir::InstructionBinaryOp>(bin_op, size,
                                                                 right, dst);
...
LeaISTile::apply(std::vector<Node *> &worklist) {
    ...
    auto displacement = om->getImmediate(0);
    auto src = om->getAddress(base, static_cast<lir::Register *>(index), scale,
                              displacement);
...
bool LeaBIDTile::matches(Node *root) {
    ...
    if (!left_reg || !right_reg) {
        ...
        tile_index = dynamic_cast<RegisterNode *>(inner_op->getRight().get());
        if (!tile_index)
            return false;
...
bool LeaISDTile::matches(Node *root) {
    ...
    if (left_reg && matchIS(left_reg, &tile_index, &tile_scale)) {
        tile_displacement = static_cast<ImmediateNode *>(op->getRight().get());
        return true;
    }
    if (right_reg && matchIS(right_reg, &tile_index, &tile_scale)) {
        tile_displacement = static_cast<ImmediateNode *>(op->getLeft().get());
        return true;
    }
